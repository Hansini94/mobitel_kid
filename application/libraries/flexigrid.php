...
//	    if (is_array($_POST) === true) {
			...
			{
				if (is_array($valid_fields)) //check valid search field array
				{
					if (in_array($this->CI->input->post('sortname'),$valid_fields))  //check whether the sortname is in validfield array
					{
			...
			if ($this->CI->input->post('query') != false && $this->CI->input->post('query') != "" && $this->CI->input->post('qtype') != FALSE && $this->CI->input->post('qtype') != "" )
			{
			...
			if($this->CI->input->post('filters') != "") {
				$this->post_info['swhere'] = $this->searchstr_validator_operator($this->CI->input->post('filters'), $this->post_info['swhere']);
				// if(is_array($filters['rules'])) {	
					// foreach($filters['rules'] as $v) {
						// if (in_array($v['field'],$valid_fields)) {
							// switch($v['op']){
								// case 'eq' : $op = '='; break;
								// case 'bt' : $op = '>'; break;
								// case 'st' : $op = '<'; break;
							...
							// if ($this->post_info['swhere'] == "") 
								// $this->post_info['swhere'] = $v['field'].' '.$op.' "%'.$v['data'].'%"';
							// else
								// $this->post_info['swhere'] .= ' OR '. $v['field'].' '.$op.' "%'.$v['data'].'%"';
						// } else $this->post_info['swhere'] = FALSE;					
					// }
				// } else $this->post_info['swhere'] = FALSE;			
			}
			else
...
/*	    }
	    else
	    {
		    ...
		    $this->post_info['limitstart'] = (($this->post_info['page']-1) * $this->CI->config->item('per_page'));
	    }
 ...
 */					
	 ...
	 * @param	search by
	 * @param	operator
	 * @param	current where condition
	 * @return	string/boolean
	 */
	private function searchstr_validator_operator($s, $swhere)
	{
		$qwery = $swhere;
	...
	private function searchstr_validator($searchstr,$searchby)
	{
		...
		else
		{
			...
			{
			        $searchstr_final = "";
				/* for one search not splitting now
