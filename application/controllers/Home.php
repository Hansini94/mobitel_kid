...
Class Home extends CI_Controller {
    ...
    public function index() {
        ...
        $data['grades'] = $this->Home_Page_model->get_grades_list();            
    ...
    function get_divisional_secretariat(){
        $district_id = $this->input->post('dis_id',TRUE);

        $this->db->select('id,vDivSecretariatName');
        $this->db->from('tbl_divisional_secretariat');
        $this->db->where('iDistrictId', $district_id);
        $this->db->where('cEnable', 'Y');
        $this->db->order_by("iOrder", "ASC");
        $records = $this->db->get('');
        ...
        foreach ($records->result() as $row)
         {
             $output .= "<option value='".$row->id."'>".$row->vDivSecretariatName."</option>";
         }
     
         echo $output;
    }
    
    function get_sin_divisional_secretariat(){
        $district_id = $this->input->post('dis_id',TRUE);

        $this->db->select('id,vDivSecretariatNameSin');
        $this->db->from('tbl_divisional_secretariat');
        $this->db->where('iDistrictId', $district_id);
        $this->db->where('cEnable', 'Y');
        $this->db->order_by("iOrder", "ASC");
        ...
        foreach ($records->result() as $row)
         {
             $output .= "<option value='".$row->id."'>".$row->vDivSecretariatNameSin."</option>";
         }
     
         echo $output;
    }
    
    function get_tam_divisional_secretariat(){
        $district_id = $this->input->post('dis_id',TRUE);

        $this->db->select('id,vDivSecretariatNameTam');
        $this->db->from('tbl_divisional_secretariat');
        $this->db->where('iDistrictId', $district_id);
        $this->db->where('cEnable', 'Y');
        $this->db->order_by("iOrder", "ASC");
        ...
        foreach ($records->result() as $row)
         {
             $output .= "<option value='".$row->id."'>".$row->vDivSecretariatNameTam."</option>";
         }
    ...
    public function save_submission() {
        ...
        $iDivSecretariatId = $this->input->post('iDivSecretariatId', TRUE); 
        $tSchool = $this->input->post('tSchool', TRUE); 
        ...
        if (!empty($_FILES['vVideo']['name'])) {
            $filename   = $nextId . "_" . time(); // 5dab1961e93a7-1571494241
            $extension  = pathinfo( $_FILES["vVideo"]["name"], PATHINFO_EXTENSION ); // jpg
            $vVideo   = $filename . "." . $extension; // 5dab1961e93a7_1571494241.jpg

            // $ext = pathinfo($_FILES['vVideo']['name'], PATHINFO_EXTENSION);
            // $video_replace = preg_replace('/[^a-zA-Z0-9]/s','_', $_FILES["vVideo"]['name']); 
            // $new_video_name = $nextId."_".time().$video_replace. "." .$ext;
            // $vVideo = $new_video_name;
        }
        if (!empty($_FILES['vImage']['name'])) {
            $filename   = $nextId . "_" . time(); // 5dab1961e93a7-1571494241
            $extension  = pathinfo( $_FILES["vImage"]["name"], PATHINFO_EXTENSION ); // jpg
            $vImage   = $filename . "." . $extension; // 5dab1961e93a7_1571494241.jpg
            
            // $ext = pathinfo($_FILES['vImage']['name'], PATHINFO_EXTENSION);
            // $image_replace = preg_replace('/[^a-zA-Z0-9]/s','_', $_FILES["vImage"]['name']);
            // $new_img_name = $nextId."_".time().$image_replace. "." .$ext;
            // $vImage = $new_img_name;
        }
        ...
        $data = array(
            ...
            'iDivSecretariatId' => $iDivSecretariatId,
            'tSchool' => $tSchool,
            'iGrade' => $iGrade,
            'vVideo' => $vVideo,   
            'vImage' => $vImage,
        ...
        // else 
        // {
        //     $response = json_decode(file_get_contents("https://www.google.com/recaptcha/api/siteverify?secret=6Ld-inYjAAAAABYzk1bqU-YU4GupEuefv67Ehe2l&response=".$captcha));
